...
/**
 * Example demonstrating how to use workspace tools with OpenAI using direct JSON schemas
 * This approach works around the OpenAI schema validation by hand-crafting valid schemas
 */
object DirectJsonToolExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      if (workspace.startContainer()) {
        ...
        // Create workspace tools with direct JSON schemas
   ...
   * Using direct JSON manipulation to ensure OpenAI compatibility
   */
  private def createWorkspaceTools(workspace: ContainerisedWorkspace): Seq[ToolFunction[_, _]] = {
    // 1. Tool for exploring files - create a handler first
    ...
    // Create a standard schema - only requiring path
    ...
    // Build the tool using the schema
    ...
    // After building the tool, we'll need to modify its JSON schema for OpenAI
  ...
  /**
   * Override the OpenAI tool definitions in the AzureToolHelper
   * This is a workaround that monkey-patches the JSON schema before sending it to OpenAI
   */
  private def patchOpenAITools(client: LLMClient, tools: Seq[ToolFunction[_, _]]): Unit = {
    if (client.isInstanceOf[OpenAIClient]) {
      val openaiTools = tools.map(_.toOpenAITool())
      
      // Patch each tool for OpenAI compatibility
      openaiTools.foreach { tool =>
        val functionObj = tool.obj("function").obj
        val parametersObj = functionObj("parameters").obj
        
        // Add missing properties based on the tool name
        tool.obj("function").obj("name").str match {
          case "explore_files" =>
            // Add recursive parameter
            parametersObj("properties").obj("recursive") = ujson.Obj(
              "type" -> ujson.Str("boolean"),
              "description" -> ujson.Str("Whether to explore recursively")
            )
          
          case "read_file" =>
            // Add startLine and endLine parameters
            parametersObj("properties").obj("startLine") = ujson.Obj(
              "type" -> ujson.Str("integer"),
              "description" -> ujson.Str("Starting line (1-indexed)")
            )
            parametersObj("properties").obj("endLine") = ujson.Obj(
              "type" -> ujson.Str("integer"),
          ...
          case "execute_command" =>
            ...
            parametersObj("properties").obj("workingDirectory") = ujson.Obj(
              "type" -> ujson.Str("string"),
              "description" -> ujson.Str("Working directory")
            )
          
          case _ => // No extra parameters needed
        }
      }
    }
  }
  ...
  private def testLLMWithTools(client: LLMClient, toolRegistry: ToolRegistry, prompt: String): Unit = {
    ...
    // Patch tools for OpenAI compatibility
    patchOpenAITools(client, toolRegistry.tools)
    ...
    client.complete(conversation, options) match {
      case Right(completion) =>
        ...
        if (assistantMessage.toolCalls.nonEmpty) {
          ...
          processLLMRequest(client, updatedConversation, CompletionOptions(), toolRegistry, depth + 1)
        } else {
