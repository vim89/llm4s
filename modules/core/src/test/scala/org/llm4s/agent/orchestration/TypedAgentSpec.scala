...
/**
 * Unit tests for TypedAgent abstraction following LLM4S testing patterns
 */
class TypedAgentSpec extends AnyFlatSpec with Matchers with ScalaFutures {
  ...
  "TypedAgent.fromUnsafeFunction" should "handle exceptions safely" in {
    val agent = TypedAgent.fromUnsafeFunction[String, Int]("unsafe-agent") { input =>
  ...
  "TypedAgent.fromFuture" should "handle Future operations properly" in {
    val agent = TypedAgent.fromFuture[String, String]("future-agent") { input =>
  ...
  "TypedAgent.fromFuture" should "handle Future failures" in {
    val agent = TypedAgent.fromFuture[String, String]("failing-future-agent") { _ =>
  ...
  "TypedAgent.constant" should "always return the same value" in {
    val agent = TypedAgent.constant[String, Int]("constant-agent", 42)
  ...
  "TypedAgent.simpleFailure" should "always fail with the specified error" in {
    val agent = TypedAgent.simpleFailure[String, Int]("failure-agent", "Always fails")
  ...
  "TypedAgent composition" should "work with different input/output types" in {
    val stringToInt = TypedAgent.fromFunction[String, Int]("string-to-int")(s => Right(s.length))
    val intToString = TypedAgent.fromFunction[Int, String]("int-to-string")(i => Right(s"Length: $i"))
  ...
  "TypedAgent with long-running operation" should "complete eventually" in {
    val slowAgent = TypedAgent.fromFuture[String, String]("slow-agent") { input =>
      Future {
